...
RUNNER_REL = Path('TEST_NUOVI_REPRO_VGG16') / 'scripts' / 'robust_fl_experiment.py'
        ...
        for alpha in matrix['non_iid_alphas']:
            for attack in matrix['attack_intensities']:
                for method in matrix['methods']:
                    ...
                    run_dir_rel = Path('TEST_NUOVI_REPRO_VGG16') / matrix['output_root'] / dataset['name'] / f"alpha_{str(alpha).replace('.', 'p')}" / attack['name'] / method['name'] / f'seed_{seed}'
                    config = {
...
def main() -> None:
    ...
    print(json.dumps({'total': len(rows), 'counts': counts, 'manifest': str(Path('TEST_NUOVI_REPRO_VGG16') / matrix['manifest_path'])}, indent=2))
