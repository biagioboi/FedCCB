...
def run_one(row: dict, slot: int, gpus: list[str] | None) -> tuple[dict, int, Path, str]:
    run_dir = resolve_repo_path(row["run_dir"])
    results_dir = run_dir / "results"
    log_path = run_dir / "runner.log"
    if results_dir.exists():
        return row, 0, log_path, "SKIPPED(existing_results)"
    ...
    return row, completed.returncode, log_path, status
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=jobs) as executor:
        ...
        for future in as_completed(futures):
            row, returncode, log_path, status = future.result()
